...
def _print_prepared_summary(prepared: PreparedData, tag: str = "[data] prepared") -> None:
    total_interactions = sum(len(seq) for seq in prepared.user_full_items.values())
    print(
        f"{tag} | "
        f"users={_fmt_count(prepared.num_users)} | "
        f"items={_fmt_count(prepared.num_items)} | "
        f"interactions={_fmt_count(total_interactions)} | "
        f"train_edges={_fmt_count(len(prepared.train_edges))} | "
        f"train_samples={_fmt_count(len(prepared.train_samples))} | "
        f"valid={_fmt_count(len(prepared.valid_eval_samples))} | "
        f"test={_fmt_count(len(prepared.test_eval_samples))}"
    )
...
def build_raw_interaction_csv(cfg: Config, force_rebuild: bool = False) -> Path:
    ...
    if out_path.exists() and not force_rebuild:
        print(f"[data] reuse raw csv: {out_path}")
        return out_path
    ...
    print(
        "[data] raw csv built | "
        f"dataset={cfg.dataset} | rows={_fmt_count(len(df))} | "
        f"users={_fmt_count(df['user_id'].nunique())} | items={_fmt_count(df['item_id'].nunique())}"
    )
    return out_path
...
def load_kcore_interactions(cfg: Config, force_rebuild: bool = False) -> pd.DataFrame:
    ...
    if path.exists() and not force_rebuild:
        print(f"[data] reuse k-core csv: {path}")
    ...
    print(
        "[data] k-core csv built | "
        f"dataset={cfg.dataset} | size={cfg.data_size_tag} | rows={_fmt_count(len(df))} | "
        f"users={_fmt_count(df['user_id'].nunique())} | items={_fmt_count(df['item_id'].nunique())}"
    )
    return df
...
def prepare_data(cfg: Config, force_rebuild: bool = False) -> PreparedData:
    ...
    if cfg.cache_path.exists() and not force_rebuild:
        print(f"[data] load cache: {cfg.cache_path}")
        with open(cfg.cache_path, "rb") as f:
            prepared = pickle.load(f)
        if not _prepared_cache_is_compatible(prepared):
            print("[data] cache schema changed; rebuilding prepared data.")
