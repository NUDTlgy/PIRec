...
def warm_up_interaction_graph(
    ...
    if graph.edge_index.numel() == 0:
        print("[warmup] skipped because the global interaction graph has no edges.")
        return
    ...
    print(
        "[warmup] graph contrastive pretraining | "
        f"epochs={epochs} | edge_drop={drop_rate:.2f} | temp={temperature:.2f}"
    )
    model.train()
    for epoch in range(1, epochs + 1):
        ...
        optimizer.step()
        print(
            f"[warmup] epoch {epoch:03d}/{epochs:03d} | "
            f"loss={float(loss.item()):.4f} | time={time.time() - t0:.2f}s"
        )
