...
from pretraining import graph_contrastive_pretrain
...
def main(cfg: Config, force_rebuild_data: bool = False, force_rediscover: bool = False) -> None:
    cfg.ensure_dirs()
    ...
    warmup_t0 = time.time()
    graph_contrastive_pretrain(model, graph, prepared.train_edges, prepared.user_train_history, cfg, device)
    _sync_if_cuda(device)
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for batch_idx, batch in enumerate(loader, start=1):
            ...
            if should_log_batch:
                print(f"[epoch {epoch:03d}] loss={float(loss.item()):.4f}")
...
if __name__ == "__main__":
    ...
    main(cfg, force_rebuild_data=args.force_rebuild_data, force_rediscover=args.force_rediscover)
